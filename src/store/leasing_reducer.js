...
import leasingAbi from 'contracts/leasingmanager/metadata.json';
import { leasingAddress,erc721Address } from 'contracts/addresses.json';
...
export const rentLease = createAsyncThunk(
    ...
    async (action,thunkAPI) => {
        try {
            ...
            const approveGasConsumed = (await erc20Contract.query.approve(account.address,{ value: 0,gasLimit: -1 },leasingAddress,CommonUtility.fromUnit(action.amount))).gasConsumed;
            const output1 = await erc20Contract.tx
                .approve({ value: 0,gasLimit: Number(approveGasConsumed) },leasingAddress,CommonUtility.fromUnit(action.amount))
