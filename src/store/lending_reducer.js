...
import lendingAbi from 'contracts/lendingmanager/metadata.json';
import { lendingAddress,erc721Address } from 'contracts/addresses.json';
    ...
    async (_,thunkAPI) => {
        try {
            ...
            if (contract == null || account == null)
                throw new Error('Contract and/or account not initialized');
...
export const lendLoan = createAsyncThunk(
    ...
    async (action,thunkAPI) => {
        try {
            ...
            const approveGasConsumed = (await erc20Contract.query.approve(account.address,{ value: 0,gasLimit: -1 },lendingAddress,CommonUtility.fromUnit(action.amount))).gasConsumed;
            const approvehash = await erc20Contract.tx
                .approve({ value: 0,gasLimit: Number(approveGasConsumed) },lendingAddress,CommonUtility.fromUnit(action.amount))
...
export const withdrawLoan = createAsyncThunk(
    ...
    async (action,thunkAPI) => {
        try {
            ...
            const amount = CommonUtility.fromUnit(action.amount * (1 + interest / 100));
